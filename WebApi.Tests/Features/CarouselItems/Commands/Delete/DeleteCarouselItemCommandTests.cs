...
namespace WebApi.Tests.Features.CarouselItems.Commands.Delete
{
    public class DeleteCarouselItemCommandTests
    {
        public class DeleteCarouselItemCommandHandlerTests
        {
            private readonly IApplicationDbContext _context;
            private readonly ILocalizationService _localizationService;
            private readonly DeleteCarouselItemCommandHandler _handler;

            public DeleteCarouselItemCommandHandlerTests()
            {
                _context = Substitute.For<IApplicationDbContext>();
                _localizationService = Substitute.For<ILocalizationService>();
                _handler = new DeleteCarouselItemCommandHandler(_context, _localizationService);
            }

            [Fact]
            public async Task Handle_ShouldSetIsActiveFalse_AndReturnSuccess_WhenItemExists()
            {
                // Arrange
                ...
                var command = new DeleteCarouselItemCommand(1);

                // Act
                var result = await _handler.Handle(command, CancellationToken.None);

                // Assert
                carouselItem.IsActive.ShouldBeFalse();
            ...
            public async Task Handle_ShouldReturnFailure_WhenItemDoesNotExist()
            {
                ...
                var command = new DeleteCarouselItemCommand(2);
            ...
            public async Task Handle_WhenSaveChangesThrows_ShouldPropagateException()
            {
                ...
                var command = new DeleteCarouselItemCommand(3);
